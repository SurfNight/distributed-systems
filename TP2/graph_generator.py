...
def parse_csv(data_file: str) -> pd.DataFrame:
    ...
    """
    df = pd.read_csv(data_file, sep="|", names=['n', 'np', 'nc', 'time'])
...
if __name__ == "__main__":
    ...
    fig = df.pivot(index='K', columns='N',
                   values='time').plot(title="Gráfico de Tempo x Número de Threads")
    fig.set_xlabel("Número de Threads")
    fig.set_ylabel("Tempo (s)")
    save_graph(fig, "./spin_lock/graph")
